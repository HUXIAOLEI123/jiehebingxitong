<template>
	<el-dialog :title="title" :visible.sync="innerVisible" :close-on-click-modal="false" width="35%" @close="close"
		append-to-body>
		...
		<el-form class="form_row" ref="form" :model="form" :rules="rules" status-icon label-width="120px">
			<el-row :gutter="20">
				...
				<el-col :span="20">
					<el-form-item label="不良反应症状:">
						<el-input v-model="form.AdverseReactionName" placeholder="请输入内容" disabled></el-input>
					</el-form-item>
				</el-col>
				<el-col :span="20" v-if="form.AdverseReactionOther">
					<el-form-item label="其他不良反应:">
						<el-input v-model="form.AdverseReactionOther" placeholder="请输入内容" disabled></el-input>
					</el-form-item>
				</el-col>

				<el-col :span="20">
					<el-form-item label="不良反应时长:">
						<el-select v-model="form.AdverseReactiondurationName" disabled>
							<el-option v-for="item in A0020" :key="item.DetailsCode" :label="item.DetailsName"
								:value="item.DetailsCode"></el-option>
						</el-select>
					</el-form-item>
				</el-col>
			...
			handlerSave() {
				this.$refs["form"].validate((valid) => {
					...
					this.$baseConfirm(`确认要${this.title}此项目吗？`, null, async () => {
						...
						const res = await addpulmonarybadreactiondisposal(badReactionId, "'" +
							managementMethod + "'")
						let {
							StatusCode,
							Message
