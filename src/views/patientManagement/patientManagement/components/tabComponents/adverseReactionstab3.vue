<template>
  <div class="room">
    ...
    <div style="width: 100%">
      <el-button class="exportBtn addBtn2" type="success" round @click="addAdverseReactions">新增不良反应</el-button>
    </div>
    <!-- 表格 -->
    <div class="tablebox">
      <vxe-table
      ...
      >
        ...
        <vxe-column field="AdverseReactionSource" title="不良反应来源"></vxe-column>
        <vxe-column field="ManagementMethod" title="不良反应处置">
          <template #default="{ row }">
            <el-link :disabled="inputData.IsClosed ? true : false" type="danger" v-if="row.IsManagement == false" @click="handlerEdit(row)">
              去处置
            </el-link>
      ...
      <!-- 弹窗 -->
      <pageEdit ref="pageEdit" @submit="sumite"></pageEdit>
...
import { pulmonarybadreactionslist, pulmonarybadreactionecharts } from '@/api/PulmonaryBadReaction.js'
...
export default {
  ...
  methods: {
    ...
    //新增不良反应
    addAdverseReactions () {
      this.$refs.pageEdit.getDetail({
        title: '新增不良反应',
        visible: true,
        opt: 'add',
        formData:{
          PersonId:this.inputData.PersonId,
          DiagnosisId:this.inputData.DiagnosisId,
        }
      })
    },
    // 图表（数据直接链接接口）
