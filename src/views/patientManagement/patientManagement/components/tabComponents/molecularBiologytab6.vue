<template>
  <div class="room">
    ...
    <div class="heardechart">
      <p>分子生物学检测病原学阳性率</p>
      <MedicinerateEchart_4 :EchartList="Echartdata"></MedicinerateEchart_4>
    </div>
    <div class="heardechart2">
      <StatisticsEcharts_1 :EchartList="Echartdata"></StatisticsEcharts_1>
    </div>
    <el-button :disabled="inputData.IsClosed ? true : false" type="primary" round @click.native="handlerEdit('biology')">检查预约</el-button>
    <!-- 表格 -->
    <div class="tablebox">
      <vxe-table
        auto-resize
      ...
      >
        ...
        <!-- 字典 A0017-->
        <vxe-column field="InspectResultName" title="检查结果" width="180"></vxe-column>
...
<script>
import MedicinerateEchart_4 from './echeatComponents/medicinerateEchart_4.vue'
import StatisticsEcharts_1 from './echeatComponents/statisticsEcharts_1.vue'
...
export default {
  components: { MedicinerateEchart_4, StatisticsEcharts_1, CheckaPpointmentEdit, Checktab5Edit, Checktab6Edit },
  props: ['inputData'],
  data () {
    return {
      radio1: '全部',
  ...
  methods: {
    ...
    async funInspectApplicationlist () {
      const res = await InspectApplicationlist({ ApplicationItemCategory: 1, SourceId: this.inputData.DiagnosisId })
    ...
    async funInspectApplicationecharts () {
      ...
      const res = await InspectApplicationecharts(diagnosisId, 1)
      let { StatusCode, Message } = res
      if (!res || StatusCode !== 200) {
        return this.$baseMessage(Message || '数据获取失败', 'error')
      }
    ...
    handlerEdit (flag, data) {
      ...
      if (flag == 'biology') {
        ...
        this.$refs.checktab5Edit.show(flag, data, this.inputData, 'A0040')
      } else if (flag == 'warning') {// 结果录入
        this.$refs.checktab5Edit.show(flag, data, this.inputData, 'A0040')
      } else if (flag == 'delete') {
        // 删除
