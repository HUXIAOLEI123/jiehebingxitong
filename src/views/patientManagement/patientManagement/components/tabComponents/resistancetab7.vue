<template>
  <div class="room">
    ...
    <div class="heardechart">
      <p>耐药检查异常率</p>
      <MedicinerateEchart_5 :EchartList="Echartdata"></MedicinerateEchart_5>
    </div>
    <div class="heardechart2">
      <StatisticsEcharts_2 :EchartList="Echartdata"></StatisticsEcharts_2>
    </div>
    <el-button :disabled="inputData.IsClosed ? true : false" type="primary" round @click.native="handlerEdit('resistance')">检查预约</el-button>
    <!-- 表格 -->
    <div class="tablebox">
      <vxe-table
        auto-resize
        ...
        <vxe-column title="操作" width="300" fixed="right">
          <template #default="{ row }">
            <div class="flex_center">
              ...
              <el-link type="warning" :disabled="inputData.IsClosed ? true : false" @click="handlerEdit('Modify2', row)" v-else>修改结果</el-link>
              <div class="Divider"></div>
...
<script>
import MedicinerateEchart_5 from './echeatComponents/medicinerateEchart_5.vue'
import StatisticsEcharts_2 from './echeatComponents/statisticsEcharts_2.vue'
...
export default {
  components: { MedicinerateEchart_5, StatisticsEcharts_2, CheckaPpointmentEdit, Checktab5Edit, Checktab6Edit }
  ,
  props: ['inputData'],
  data () {
    return {
  ...
  methods: {
    ...
    async funInspectApplicationlist () {
      const res = await InspectApplicationlist({ ApplicationItemCategory: 4, SourceId: this.inputData.DiagnosisId })
    ...
    async funInspectApplicationecharts () {
      ...
      const res = await InspectApplicationecharts(diagnosisId, 4)
      let { StatusCode, Message } = res
      if (!res || StatusCode !== 200) {
        return this.$baseMessage(Message || '数据获取失败', 'error')
      }
    ...
    handlerEdit (flag, data) {
      ...
      if (flag == 'resistance') {
        ...
        this.$refs.checktab5Edit.show(flag, data, this.inputData, 'A0024')
