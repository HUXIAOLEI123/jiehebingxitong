<template>
  <div class="room">
    ...
    <div class="heardechart">
      <p>胸片异常率</p>
      <MedicinerateEchart_6 :EchartList="Echartdata"></MedicinerateEchart_6>
    </div>
    <div class="heardechart2">
      <StatisticsEcharts_3 :EchartList="Echartdata"></StatisticsEcharts_3>
    </div>

    <el-button :disabled="inputData.IsClosed ? true : false" type="primary" round @click.native="handlerEdit('chest')">检查预约</el-button>
...
<script>
import MedicinerateEchart_6 from './echeatComponents/medicinerateEchart_6.vue'
import StatisticsEcharts_3 from './echeatComponents/statisticsEcharts_3.vue'
...
export default {
  components: { MedicinerateEchart_6, StatisticsEcharts_3, CheckaPpointmentEdit, Checktab5Edit, Checktab6Edit }
  ,
  props: ['inputData'],
  data () {
    return {
  ...
  methods: {
    ...
    async funInspectApplicationlist () {
      const res = await InspectApplicationlist({ ApplicationItemCategory: 3, SourceId: this.inputData.DiagnosisId })
    ...
    async funInspectApplicationecharts () {
      ...
      const res = await InspectApplicationecharts(diagnosisId, 3)
      let { StatusCode, Message } = res
