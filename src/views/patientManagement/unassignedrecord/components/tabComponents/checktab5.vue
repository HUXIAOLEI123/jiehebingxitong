<template>
  <div class="room">
    ...
    <div class="heardechart">
      <p>痰菌检查病原学阳性率</p>
      <MedicinerateEchart_3 :EchartList="Echartdata"></MedicinerateEchart_3>
    </div>
    <div class="heardechart2">
      <StatisticsEcharts :EchartList="Echartdata"></StatisticsEcharts>
    </div>
    <el-button :disabled="inputData.IsClosed ? true : false" type="primary" round @click="handlerEdit('Phlegm')">检查预约</el-button>
    <!-- 表格 -->
    <div class="tablebox">
      <vxe-table
        auto-resize
...
<script>
import MedicinerateEchart_3 from './echeatComponents/medicinerateEchart_3.vue'
import StatisticsEcharts from './echeatComponents/statisticsEcharts.vue'
...
export default {
  components: { MedicinerateEchart_3, StatisticsEcharts, CheckaPpointmentEdit, Checktab5Edit, Checktab6Edit }
  ,
  props: ['inputData'],
  data () {
    return {
  ...
  methods: {
    ...
    async funInspectApplicationlist () {
      const res = await InspectApplicationlist({ ApplicationItemCategory: 2, SourceId: this.inputData.DiagnosisId })
    ...
    async funInspectApplicationecharts () {
      let diagnosisId = this.copyData[0].DiagnosisId || this.copyData[0].SourceId
      const res = await InspectApplicationecharts(diagnosisId, 2)
      let { StatusCode, Message } = res
    ...
    handlerEdit (flag, data) {
      ...
      if (flag == 'Phlegm') {
        ...
        this.$refs.checktab5Edit.show(flag, data, this.inputData, 'A0015')
    ...
    async checkaPpointmentEdit (flag, form, hasinfo) {
      ...
      if (flag == 'Phlegm') {
        ...
        let sourceId = this.copyData[0].DiagnosisId || this.copyData[0].SourceId
