<template>
	<el-dialog :title="title" :visible.sync="innerVisible" top="20px" :close-on-click-modal="false" width="80%"
		...
		<div slot="title" class="dialog-title">
			...
			<span class="title-text">新增随访</span>
			...
			<el-row>
				...
				<el-col :span="8">
					<el-form-item label="随访日期" prop="FirstVisitDate">
						<el-date-picker v-model="form.FirstVisitDate" align="center" value-format="yyyy-MM-dd"
			...
			<el-row>
				<el-col :span="8">
					<el-form-item label="日吸烟量">
						<el-input v-model="form.Smoking" oninput="value=value.replace(/[^\d]/g,'')" placeholder="请输入内容">
				...
				<el-col :span="8">
					<el-form-item label="建议日吸烟量">
						<el-input v-model="form.ProposalSmoking" oninput="value=value.replace(/[^\d]/g,'')"
							placeholder="请输入内容">
							<i slot="suffix" style="font-style: normal">支</i>
						</el-input>
					</el-form-item>
				...
				<el-col :span="8">
					<el-form-item label="取药时间" prop="DrugTime">
						<el-date-picker v-model="form.DrugTime" align="center" value-format="yyyy-MM-dd" type="date"
			...
			<el-row>
				...
				<el-col :span="8">
					<el-form-item label="下次随访日期">
						<el-date-picker v-model="form.NextVisitDate" align="center" type="date"
	...
	export default {
		...
		data() {
			return {
				...
				DrugsType: [],
				form: {
					FollowDoctorName: JSON.parse(localStorage.getItem('Patientsdetails')).ResponsibilityName,
				},
				Symptom: [],
				hasinfo: [],
				dictarr: [],
		...
		methods: {
			...
			handlerSave(row) {
				...
				this.$refs["form"].validate((valid) => {
					if (!valid) return;
					this.$baseConfirm(`确认要新增随访？`, null, async () => {
