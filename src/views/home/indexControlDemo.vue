<template>
  <div id="screen" :style="{ width: `${style.width}px`, height: `${style.height}px`, transform: `${style.transform}` }">
    <div class="bg">
      <!-- 标题 -->
      <div>
        <div class="head_title">结核病督导综合监测管理平台(演示数据)</div>
        ...
        <div class="content_left">
          ...
          <div class="number_of_people">
            <div>
              ...
              <div class="f30 c00FFFF">15315</div>
            </div>
            <div>
              <img src="../../assets/indexControl/dudao.png" alt="" />
              <div class="f14 cFFF">结核病督导人数</div>
              <div class="f30 c00FFFF">15278</div>
            </div>
            <div>
              <img src="../../assets/indexControl/xiaolv.png" alt="" />
              <div class="f14 cFFF">规范管理率</div>
              <div class="f30 c00FFFF">95.35%</div>
            </div>
          </div>
          <div class="flexC content_left_2">
            <div style="width: 50%" @click="itemClick('结核病情分布情况')" :class="[active == '结核病情分布情况' ? 'active' : 'noActive']">
              结核病情分布情况
            </div>
          ...
          <div style="width: 100%; height: 150px">
            <div id="Cake" style="width: 100%; height: 150px" v-if="active == '结核病情分布情况'"></div>
...
export default {
  ...
  methods: {
    ...
    async setEachartsData (name, data) {
      ...
      let tableData = [
        { RegionName: "A市", MDRRate: "100", ManagementRate: "100" },
        { RegionName: "C市", MDRRate: "95", ManagementRate: "97" },
        { RegionName: "D市", MDRRate: "95", ManagementRate: "97" },
        { RegionName: "X市", MDRRate: "93", ManagementRate: "92" },
        { RegionName: "E市", MDRRate: "86", ManagementRate: "93" },
        { RegionName: "H市", MDRRate: "94", ManagementRate: "98" },
        { RegionName: "J市", MDRRate: "96", ManagementRate: "95" },
        { RegionName: "U市", MDRRate: "98", ManagementRate: "97" },
        { RegionName: "Y市", MDRRate: "94", ManagementRate: "99" },
        { RegionName: "Z市", MDRRate: "99", ManagementRate: "91" },
      ]
    ...
    async setCharData () {
      ...
      this.$nextTick(() => {
        this.$set(this.dashboardSupervisionoverviewData, 'Patients', 4234)
        this.$set(this.dashboardSupervisionoverviewData, 'Supervision', 543)
        this.$set(this.dashboardSupervisionoverviewData, 'ManagementRate', 6543)
        this.$set(this.dashboardSupervisionoverviewData, 'Male', 332)
        this.$set(this.dashboardSupervisionoverviewData, 'Female', 5643)
        this.$set(this.dashboardSupervisionoverviewData, 'City', 2342)
        this.$set(this.dashboardSupervisionoverviewData, 'Town', 6443)
        this.$set(this.dashboardSupervisionoverviewData, 'MedicationConversions', 754)
        this.$set(this.dashboardSupervisionoverviewData, 'BadReactions', 4322)
        this.$set(this.dashboardSupervisionoverviewData, 'Visits', 3342)
        this.$set(this.dashboardSupervisionoverviewData, 'Inspections', 6543)
        this.$forceUpdate()
      })
      let option = {}
      option = {
    ...
    async setDDYCQSData () {
      ...
      for (let item of xData) {
        ...
        firstIndustry.push(Math.trunc(Math.random() * (100 - 1 + 1) + 33))
        twoIndustry.push(Math.trunc(Math.random() * (100 - 1 + 1) + 33))
        thirdIndustry.push(Math.trunc(Math.random() * (100 - 1 + 1) + 33))
      }
      option = {
    ...
    async setSFZSWCLData () {
      ...
      for (let item of Xdata) {
        ...
        column.push(Math.trunc(Math.random() * (8000 - 1000 + 1) + 8000));
        lineData.push(Math.trunc(Math.random() * (54 - 1 + 1) + 44));
      }
      option = {
        ...
        series: [
          {
            ...
            itemStyle: {
              color: {
                ...
                y2: 1,
                colorStops: [{
                  offset: 0, color: '#72F5E4' // 0% 处的颜色
                }, {
                  offset: 1, color: '#5A92E5' // 100% 处的颜色
                }],
    ...
    async setFYYCBLData () {
      ...
      for (let item of Xdata) {
        column.push(Math.trunc(Math.random() * (8000 - 1000 + 1) + 8000));
        lineData.push(Math.trunc(Math.random() * (33 - 1 + 1) + 43));
      }
      option = {
        backgroundColor: '#050f29',
        tooltip: {
    ...
    async setBLFYSBLData () {
      ...
      let Xdata = ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月'];
      for (let item of Xdata) {
        column.push(Math.trunc(Math.random() * (44 - 22 + 1) + 11));
        lineData.push(Math.trunc(Math.random() * (88 - 10 + 1) + 11));
      }
      option = {
        backgroundColor: '#050f29',
        tooltip: {
          ...
          formatter: function (params) {
            for (let item of res.Response) {
              if (item.RegionName == params.name) {
                return `<h2>` + params.name + `</h2>` + `<br/>` +
                  `管理规范率:` + item.ManagementRate + `%<br/>` +
                  `管理人数:` + item.ManagementPatients + `人<br/>` +
                  `患者总数:` + item.Patients + `人`
              }
            }
          },
        },
